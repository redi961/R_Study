...
seq(1, 10, 2) #1 에서 10까지 2씩 증가하는 벡터 생성

# rep() 함수를 이용한 벡터 생성
rep(1:3, 3) # 지정횟수만큼 같은 행동을 반복함함
rep(1:3, each = 3) # Each -> 반복횟수

# 실습: union(), setdiff() 그리고 intersect() 함수를 이용한 벡터 자료 처리 
x <- c(1, 3, 5, 7)
y <- c(3, 5)
union(x, y)
setdiff(x, y) # x에는있고 y에는 없는값 추출
intersect(x, y)
...
m <- matrix(c(1:10), nrow = 2) #2행의 자료 생성성
m


# 실습: 행과 열의 수가 일치하지 않는 경우
m <- matrix(c(1:11), nrow = 2) #길이는 반드시 행의 배수가 되야함함
m # 모자라는 데이터는 첫데이터부터 다시 넣음음
...
x1 <- c(m, 40, 50:52)
x2 <- c(30, 5, 6:8)
mr <- rbind(x1, x2) #rowbind
mr #2행의 부족한 자료수만큼 첫 데이터부터 반복되어 들어감감

# 실습: 열 묶음으로 행렬 객체 생성하기 
mc <- cbind(x1, x2) #colbind
mc
...
# 실습: ;apply() 함수 적용하기 
apply(x, 1, max) #중앙값이 1-> 행단위 // 2->열단위 /// x행렬내에서 행단위로 최대값을 구하는것
apply(x, 1, min)
apply(x, 2, mean) #mean -> 평균값

# 실습: 사용자 정의 함수 적용하기 
f <- function(x) {
  x * c(1, 2, 3)
}
result <- apply(x, 1, f) #1,2,3 곱을 행단위로 곱한뒤 열단위로 정렬함
result

##### 별도의 입력이 없을시 행렬의 출력을 열을 우선시함#####

# 실습: 열 우선 순서로 사용자 정의 함수 적용하기 
result <- apply(x, 2, f) #1,2,3곱을 열단위로 곱한뒤 열단위로 정렬함
result
...
name <- c("hong", "lee", "kim")
pay <- c(150, 250, 300)
## 각 벡터 개체의 길이가 다를시 오류가 발생함 길이 주의의
