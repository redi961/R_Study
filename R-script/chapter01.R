...
boolean
sum(10, 20, 20)
sum(10, 20, 20, NA) # NA -> 결측치 자료형형 (인수중 NA가 포함될시 반드시 결과값은 NA가 나옴)
sum(10, 20, 20, NA, na.rm = TRUE) #na.rm = TRUE -> na 결측치 제거 (NA제거)
ls()
...
result <- x * 3 #3이 문자열이라 실행되지 않음음
result <- as.numeric(x) * 3 #x를 숫자형으로 변환환
...
plot(gender) #plot -> 그래프를 그리는 메소드 -> 오류가남
